#!/usr/bin/env node
// generar_esquema.cjs
// Ejecutar: node generar_esquema.cjs
...
const OUTPUT_FILE = 'D:\\empresas\\scripts\\generar_esquema\\esquema_completo.txt';
...
// ====== Árbol ======
async function buildTreeUnderPrefix(rootAbsPath, prefix, isLastAtTop) {
  const lines = [];
  const files = [];
  ...
  async function walk(dir, curPrefix) {
    ...
    for (const f of filesHere) {
      ...
      const isLast = i === total;
      const branch = isLast ? '└── ' : '├── ';
      lines.push(`${curPrefix}${branch}${f.name}`);
...
async function readFileSmart(absPath) {
  ...
  // Caso XLSX: mostrar primera fila formateada
...
(async () => {
  ...
  for (let idx = 0; idx < existingRoots.length; idx++) {
    ...
    const { lines: subLines, files: subFiles } = await buildTreeUnderPrefix(root, topPrefix, isLast);
    lines.push(...subLines);
    files.push(...subFiles);
  }
  ...
  for (const f of files) {
    chunks.push('\n--------------------------------------------\n');
    chunks.push(`${normWin(f)}:`);
    chunks.push(await readFileSmart(f));
  }
